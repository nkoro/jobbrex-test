# eval_dashboard.py — EvalOps by JobRex
# Professional investor-grade dashboard redesign
...
<style>
  ...
  div[data-testid="stDataFrameResizable"] {{ border:1px solid {BORDER}; border-radius:10px; overflow:hidden; }}
  ...
  [data-testid="stTextInput"] input {{ border-radius:8px !important; border-color:{BORDER} !important; }}
  [data-testid="stTextArea"] textarea {{ border-radius:8px !important; border-color:{BORDER} !important; }}
  [data-testid="stExpander"] {{ border:1px solid {BORDER} !important; border-radius:10px !important; }}
</style>
""", unsafe_allow_html=True)

POLICY_FILE = "flight_policy.md"
...
def check_policy_staleness(df):
    try:
        policy_ts = datetime.fromtimestamp(os.path.getmtime(POLICY_FILE))
        if "run_time" in df.columns and not df.empty:
            last_eval = pd.to_datetime(df["run_time"].max())
            return policy_ts > last_eval.to_pydatetime()
    except:
        pass
    return False

def append_to_policy(new_rule, author="analyst"):
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M")
    current   = load_policy()
    backup    = f"flight_policy_{timestamp.replace(':','-').replace(' ','_')}.bak"
    with open(backup, "w") as f:
        f.write(current)
    with open(POLICY_FILE, "a") as f:
        f.write(f"\n- **[{timestamp}] [{author}]:** {new_rule}")
    return backup

def fmt_violation(v):
    """Turn 'Time_Window_Rule' → 'Time Window Rule', keep 'None' and '—' as-is."""
...
with tab1:
    if df.empty:
        st.info("No evaluation data yet. Run `python run_evals.py` to generate results.")
        st.stop()

    if check_policy_staleness(df):
        st.warning("Policy was updated after the last eval run — scores may be stale.")
...
with tab2:
    st.markdown(f"""<div class="card" style="margin-bottom:1.5rem;">
      ...
      <div class="card-caption" style="margin-bottom:0;">Every prompt passes through <strong>GPT-2 locally</strong>
      before reaching Groq. The last-token hidden state is L2-normalised and its variance used as a
      <strong>semantic entropy score</strong>. Low entropy → fast 8b model. High entropy → powerful 70b model.
      This gives real cost-routing with measurable quality control.</div>
    </div>""", unsafe_allow_html=True)

    if df.empty or "ka2l_destination" not in df.columns:
        st.info("No routing data. Re-run `python run_evals.py`.")
    else:
        easy = int((df["ka2l_destination"] == "agent_easy").sum())
        hard = int((df["ka2l_destination"] == "agent_hard").sum())
        avg_var = df["ka2l_variance"].replace(-1, float("nan")).mean()
        THRESHOLD = 0.5
        ...
        st.markdown(f"""<div class="kpi-grid">
          ...
          {kpi("Avg Entropy σ²", f"{avg_var:.5f}" if not pd.isna(avg_var) else "—", "L2-normalised GPT-2 variance", "slate")}
          {kpi("Routing Threshold", str(THRESHOLD), "above → agent_hard", "slate")}
          {kpi("Cost Saving", f"{(easy/len(df))*100:.0f}%", "cases on cheap model", "emerald")}
        </div>""", unsafe_allow_html=True)
        ...
        with c1:
            st.markdown('<div class="card"><div class="card-title">Model Selected per Case</div><div class="card-caption">Indigo = 8b-instant · Amber = 70b-versatile</div>', unsafe_allow_html=True)
            rdf = df[["id","ka2l_destination","ka2l_variance","input"]].copy()
            rdf["color"] = rdf["ka2l_destination"].map({"agent_easy": INDIGO, "agent_hard": AMBER, "unknown": SLATE_5})
            rdf["label"] = rdf["ka2l_destination"].map({"agent_easy": "8b", "agent_hard": "70b", "unknown": "?"})
            fig3 = go.Figure()
            for _, r in rdf.iterrows():
                fig3.add_trace(go.Bar(x=[f"Case {r['id']}"], y=[1], marker_color=r["color"],
                    text=r["label"], textposition="inside", textfont=dict(size=13, color="white"),
                    hovertext=f"σ²={r['ka2l_variance']:.5f}\n{str(r['input'])[:55]}", showlegend=False))
            fig3.update_layout(**CHART_LAYOUT, height=240, yaxis={**BASE_YAXIS, "visible": False}, bargap=0.35)
            st.plotly_chart(fig3, use_container_width=True, config={"displayModeBar": False})
            st.markdown(f'<span class="badge badge-easy">8b-instant</span> &nbsp; <span class="badge badge-hard">70b-versatile</span>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)

        with c2:
            st.markdown(f'<div class="card"><div class="card-title">Semantic Entropy per Case</div><div class="card-caption">Below threshold ({THRESHOLD}) = agent_easy · above = agent_hard</div>', unsafe_allow_html=True)
            fig4 = go.Figure()
            fig4.add_trace(go.Scatter(x=[f"Case {i}" for i in df["id"]], y=df["ka2l_variance"].replace(-1, None),
        ...
        log = [{"Case": f"Case {r['id']}", "Prompt": str(r.get("input",""))[:60]+"…",
                "Entropy": f"{r.get('ka2l_variance',-1):.6f}",
                "Route": "🟢 easy" if r.get("ka2l_destination")=="agent_easy" else "🟡 hard" if r.get("ka2l_destination")=="agent_hard" else "❓",
...
# ══════════════════════════════════════════════════════════════════════════════
with tab3:
    st.markdown('<div class="card" style="margin-bottom:1.5rem;"><div class="card-title">Agent Playground</div><div class="card-caption">Send a live prompt through the full KA2L → Groq pipeline using the current policy.</div></div>', unsafe_allow_html=True)
    ...
    with ci:
        user_input = st.text_input("", placeholder="Try: Change my flight TKT-123 to Pyongyang next Tuesday", label_visibility="collapsed")
    with cb:
        run = st.button("Run →", use_container_width=True)

    if run and user_input:
        with st.spinner("KA2L routing → Groq inference…"):
            try:
                result = run_flight_crew(user_input)
                r = result["routing"]
...
# ══════════════════════════════════════════════════════════════════════════════
with tab4:
    st.markdown('<div class="card" style="margin-bottom:1.5rem;"><div class="card-title">Policy Editor</div><div class="card-caption">Select a failed case, diagnose the failure, and append a new rule. The agent reads the policy fresh on every call — no restart needed.</div></div>', unsafe_allow_html=True)

    if df.empty:
        st.info("Run `python run_evals.py` first.")
    else:
        ...
        with cr:
            ...
            new_rule = st.text_area("New policy rule", height=110, value=suggestion,
                placeholder="e.g., Passengers who cancel within 24 hours may receive a full refund regardless of fare class.", key="new_rule")
            if st.button("Append to Policy"):
                if new_rule.strip():
                    bak = append_to_policy(new_rule, author)
                    st.success(f"Policy updated. Backup: `{bak}`")
                    st.info("Re-run `python run_evals.py` then check Regression tab.")
                else:
...
# ══════════════════════════════════════════════════════════════════════════════
with tab5:
    st.markdown('<div class="card" style="margin-bottom:1.5rem;"><div class="card-title">Regression Tracker</div><div class="card-caption">Compare two eval runs to confirm a policy fix improved scores without introducing regressions.</div></div>', unsafe_allow_html=True)

    result_files = sorted([f for f in os.listdir(".") if f.startswith("eval_results") and f.endswith(".json")])

    if len(result_files) < 2:
        st.info("You need at least 2 eval runs. Make a policy change, re-run `python run_evals.py`, then return here.")
        st.code("python run_evals.py", language="bash")
    else:
        ca2, cb2 = st.columns(2, gap="large")
        baseline_f = ca2.selectbox("Baseline (before fix)", result_files, index=0)
        current_f  = cb2.selectbox("Current (after fix)",   result_files, index=len(result_files)-1)

        if st.button("Compare Runs"):
            df_base = pd.DataFrame(json.load(open(baseline_f)))
            df_curr = pd.DataFrame(json.load(open(current_f)))
